...
- `date.``replace`(*year=self.year*, *month=self.month*, *day=self.day*)
  ...
  返回一个具有同样值的日期，除非通过任何关键字参数给出了某些形参的新值。示例:>>>`>>> from datetime import date >>> d = date(2002, 12, 31) >>> d.replace(day=26) datetime.date(2002, 12, 26) `
...
- `datetime.``strftime`(*format*)
  ...
  返回一个由显式格式字符串所指明的代表日期和时间的字符串，要获取格式指令的完整列表，请参阅 [strftime() 和 strptime() 的行为](https://docs.python.org/zh-cn/3/library/datetime.html#strftime-strptime-behavior)。
...
- `time.``resolution`
  ...
  两个不相等的 [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) 对象之间可能的最小间隔，`timedelta(microseconds=1)`，但是请注意 [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) 对象并不支持算术运算。
...
- `tzinfo.``utcoffset`(*dt*)
  ...
  将本地时间与 UTC 时差返回为一个 [`timedelta`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timedelta) 对象，如果本地时区在 UTC 以东则为正值。 如果本地时区在 UTC 以西则为负值。这表示与 UTC 的 *总计* 时差；举例来说，如果一个 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 对象同时代表时区和 DST 调整，则 [`utcoffset()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.utcoffset) 应当返回两者的和。 如果 UTC 时差不确定则返回 `None`。 在其他情况下返回值必须为一个 [`timedelta`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timedelta) 对象，其取值严格限制于 `-timedelta(hours=24)` 和 `timedelta(hours=24)` 之间（差值的幅度必须小于一天）。 大多数 [`utcoffset()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.utcoffset) 的实现看起来可能像是以下两者之一:`return CONSTANT                 # fixed-offset class return CONSTANT + self.dst(dt)  # daylight-aware class `如果 [`utcoffset()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.utcoffset) 返回值不为 `None`，则 [`dst()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.dst) 也不应返回 `None`。默认的 [`utcoffset()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.utcoffset) 实现会引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3/library/exceptions.html#NotImplementedError)。*在 3.7 版更改:* UTC 时差不再限制为一个整数分钟值。

- `tzinfo.``dst`(*dt*)

  将夏令时（DST）调整返回为一个 [`timedelta`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timedelta) 对象，如果 DST 信息未知则返回 `None`。如果 DST 未启用则返回 `timedelta(0)`。 如果 DST 已启用则将差值作为一个 [`timedelta`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timedelta) 对象返回（参见 [`utcoffset()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.utcoffset) 了解详情）。 请注意 DST 差值如果可用，就会直接被加入 [`utcoffset()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.utcoffset) 所返回的 UTC 时差，因此无需额外查询 [`dst()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.dst) 除非你希望单独获取 DST 信息。 例如，[`datetime.timetuple()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.timetuple) 会调用其 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.tzinfo) 属性的 [`dst()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.dst) 方法来确定应该如何设置 `tm_isdst` 旗标，而 [`tzinfo.fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 会调用 [`dst()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.dst) 来在跨越时区时处理 DST 的改变。一个可以同时处理标准时和夏令时的 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类的实例 *tz* 必须在此情形中保持一致：`tz.utcoffset(dt) - tz.dst(dt)`必须为具有同样的 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类实例且 `dt.tzinfo == tz` 的每个 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 对象 *dt* 返回同样的结果，此表达式会产生时区的“标准时差”，它不应取决于具体日期或时间，只取决于地理位置。 [`datetime.astimezone()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.astimezone) 的实现依赖此方法，但无法检测违反规则的情况；确保符合规则是程序员的责任。 如果一个 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类不能保证这一点，也许可以重载 [`tzinfo.fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 的默认实现以便在任何情况下与 `astimezone()` 正确配合。大多数 [`dst()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.dst) 的实现可能会如以下两者之一:`def dst(self, dt):    # a fixed-offset class:  doesn't account for DST    return timedelta(0) `或者：`def dst(self, dt):    # Code to set dston and dstoff to the time zone's DST    # transition times based on the input dt.year, and expressed    # in standard local time.     if dston <= dt.replace(tzinfo=None) < dstoff:        return timedelta(hours=1)    else:        return timedelta(0) `默认的 [`dst()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.dst) 实现会引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3/library/exceptions.html#NotImplementedError)。*在 3.7 版更改:* DST 差值不再限制为一个整数分钟值。

- `tzinfo.``tzname`(*dt*)

  将对应于 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 对象 *dt* 的时区名称作为字符串返回。 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#module-datetime) 模块没有定义任何字符串名称相关内容，也不要求名称有任何特定含义。 例如 "GMT", "UTC", "-500", "-5:00", "EDT", "US/Eastern", "America/New York" 都是有效的返回值。 如果字符串名称未知则返回 `None`。 请注意这是一个方法而不是一个固定的字符串，这主要是因为某些 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类可能需要根据所传入的特定 *dt* 值返回不同的名称，特别是当 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 类要负责处理夏令时的时候。默认的 [`tzname()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.tzname) 实现会引发 [`NotImplementedError`](https://docs.python.org/zh-cn/3/library/exceptions.html#NotImplementedError)。

这些方法会被 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 或 [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) 对象调用，用来与它们的同名方法相对应。 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 对象会将自身作为传入参数，而 [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) 对象会将 `None` 作为传入参数。 这样 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类的方法应当准备好接受 *dt* 参数值为 `None` 或是 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 类的实例。

当传入 `None` 时，应当由类的设计者来决定最佳回应方式。 例如，返回 `None` 适用于希望该类提示时间对象不参与 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 协议处理。 让 `utcoffset(None)` 返回标准 UTC 时差也许会更有用处，因为并没有其他可用于发现标准时差的约定惯例。

当传入一个 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 对象来回应 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 方法时，`dt.tzinfo` 与 *self* 是同一对象。 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 方法可以依赖这一点，除非用户代码直接调用了 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 方法。 此行为的目的是使得 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 方法将 *dt* 解读为本地时间，而不需要担心其他时区的相关对象。

还有一个额外的 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 方法，某个子类可能会希望重载它：

- `tzinfo.``fromutc`(*dt*)

  此方法会由默认的 [`datetime.astimezone()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.astimezone) 实现来调用。 当被其调用时，`dt.tzinfo` 为 *self*，并且 *dt* 的日期和时间数据会被视为表示 UTC 时间，[`fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 的目标是调整日期和时间数据，返回一个等价的 datetime 来表示 *self* 的本地时间。大多数 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类应该能够毫无问题地继承默认的 [`fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 实现。 它的健壮性足以处理固定差值的时区以及同时负责标准时和夏令时的时区，对于后者甚至还能处理 DST 转换时间在各个年份有变化的情况。 一个默认 [`fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 实现可能无法在所有情况下正确处理的例子是（与 UTC 的）标准时差取决于所经过的特定日期和时间，这种情况可能由于政治原因而出现。 默认的 `astimezone()` 和 [`fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 实现可能无法生成你希望的结果，如果这个结果恰好是跨越了标准时差发生改变的时刻当中的某个小时值的话。忽略针对错误情况的代码，默认 [`fromutc()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo.fromutc) 实现的行为方式如下:`def fromutc(self, dt):    # raise ValueError error if dt.tzinfo is not self    dtoff = dt.utcoffset()    dtdst = dt.dst()    # raise ValueError if dtoff is None or dtdst is None    delta = dtoff - dtdst  # this is self's standard offset    if delta:        dt += delta   # convert to standard local time        dtdst = dt.dst()        # raise ValueError if dtdst is None    if dtdst:        return dt + dtdst    else:        return dt `

在以下 [`tzinfo_examples.py`](https://docs.python.org/zh-cn/3/_downloads/6b45dc135219d1404be49d606589a11d/tzinfo_examples.py) 文件中有一些 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 类的例子：
...
请注意同时负责标准时和夏令时的 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类在每年两次的 DST 转换点上会出现不可避免的微妙问题。具体而言，考虑美国东部时区 (UTC -0500)，它的 EDT 从三月的第二个星期天 1:59 (EST) 之后一分钟开始，并在十一月的第一天星期天 1:59 (EDT) 之后一分钟结束:
...
当 DST 结束时（见 "end" 行），会有更糟糕的潜在问题：本地时间值中有一个小时是不可能没有歧义的：夏令时的最后一小时。 即以北美东部时间表示当天夏令时结束时的形式为 5:MM UTC 的时间。 本地时钟从 1:59 (夏令时) 再次跳回到 1:00 (标准时)。 形式为 1:MM 的本地时间就是有歧义的。 此时 `astimezone()` 是通过将两个相邻的 UTC 小时映射到两个相同的本地小时来模仿本地时钟的行为。 在这个北美东部时间的示例中，形式为 5:MM 和 6:MM 的 UTC 时间在转换为北美东部时间时都将被映射到 1:MM，但前一个时间会将 [`fold`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.fold) 属性设为 0 而后一个时间会将其设为 1。 例如，在 2016 年秋季时钟往回调整时，我们得到:
...
请注意不同的 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 实例仅通过 [`fold`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.fold) 属性值来加以区分，它们在比较时会被视为相等。

不允许时间显示存在歧义的应用需要显式地检查 [`fold`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.fold) 属性的值，或者避免使用混合式的 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类；当使用 [`timezone`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone) 或者任何其他固定差值的 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 子类例如仅表示 EST（固定差值 -5 小时）或仅表示 EDT（固定差值 -4 小时）的类时是不会有歧义的。

参见

- [dateutil.tz](https://dateutil.readthedocs.io/en/stable/tz.html)

  [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#module-datetime) 模块有一个基本 [`timezone`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone) 类（用来处理任意与 UTC 的固定时差）及其 [`timezone.utc`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone.utc) 属性（一个 UTC 时区实例）。*dateutil.tz* 库将 *IANA 时区数据库* (又名 Olson 数据库) 引入 Python 并推荐使用。
...
## [`timezone`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone) 对象

[`timezone`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone) 类是 [`tzinfo`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.tzinfo) 的子类，它的每个实例都代表一个以与 UTC 的固定时差来定义的时区。

此类的对象不可被用于代表某些特殊地点的时区信息，这些地点在一年的不同日期会使用不同的时差，或是在历史上对民用时间进行过调整。

- *class* `datetime.``timezone`(*offset*, *name=None*)

  *offset* 参数必须指定为一个 [`timedelta`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timedelta) 对象，表示本地时间与 UTC 的时差。 它必须严格限制于 `-timedelta(hours=24)` 和 `timedelta(hours=24)` 之间，否则会引发 [`ValueError`](https://docs.python.org/zh-cn/3/library/exceptions.html#ValueError)。*name* 参数是可选的。 如果指定则必须为一个字符串，它将被用作 [`datetime.tzname()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.tzname) 方法的返回值。*3.2 新版功能.**在 3.7 版更改:* UTC 时差不再限制为一个整数分钟值。

- `timezone.``utcoffset`(*dt*)

  返回当 [`timezone`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone) 实例被构造时指定的固定值。*dt* 参数会被忽略。 返回值是一个 [`timedelta`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timedelta) 实例，其值等于本地时间与 UTC 之间的时差。*在 3.7 版更改:* UTC 时差不再限制为一个整数分钟值。

- `timezone.``tzname`(*dt*)

  返回当 [`timezone`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.timezone) 实例被构造时指定的固定值。如果没有在构造器中提供 *name*，则 `tzname(dt)` 所返回的名称将根据 `offset` 值按以下规则生成。 如果 *offset* 为 `timedelta(0)`，则名称为“UTC”，否则为字符串 `UTC±HH:MM`，其中 ± 为 `offset` 的正负符号，HH 和 MM 分别为表示 `offset.hours` 和 `offset.minutes` 的两个数码。*在 3.6 版更改:* 由 `offset=timedelta(0)` 生成的名称现在为简单的 'UTC' 而不再是 `'UTC+00:00'`。
...
- `timezone.``fromutc`(*dt*)
  ...
  返回 `dt + offset`。 *dt* 参数必须为一个感知型 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 实例，其中 `tzinfo` 值设为 `self`。
...
[`date`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.date), [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 和 [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) 对象都支持 `strftime(format)` 方法，可用来创建由一个显式格式字符串所控制的表示时间的字符串。

相反地，[`datetime.strptime()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.strptime) 类会根据表示日期和时间的字符串和相应的格式字符串来创建一个 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 对象。

下表提供了 `strftime()` 与 `strptime()` 的高层级比较：

|          | `strftime`                                                   | `strptime`                                                   |
| :------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| 用法     | 根据给定的格式将对象转换为字符串                             | 将字符串解析为给定相应格式的 [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) 对象 |
| 方法类型 | 实例方法                                                     | 类方法                                                       |
| 方法     | [`date`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.date); [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime); [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) | [`datetime`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime) |
...
这些代码可能不是在所有平台上都可与 `strftime()` 方法配合使用。 ISO 8601 年份和 ISO 8601 星期指令并不能与上面的年份和星期序号指令相互替代。 调用 `strptime()` 时传入不完整或有歧义的 ISO 8601 指令将引发 [`ValueError`](https://docs.python.org/zh-cn/3/library/exceptions.html#ValueError)。

对完整格式代码集的支持在不同平台上有所差异，因为 Python 要调用所在平台的 C 库的 `strftime()` 函数，而不同平台的差异是很常见的。 要查看你所用平台所支持的完整格式代码集，请参阅 *[strftime(3)](https://manpages.debian.org/strftime(3))* 文档。

*3.6 新版功能:* 增加了 `%G`, `%u` 和 `%V`。

### 技术细节

总体而言，`d.strftime(fmt)` 类似于 [`time`](https://docs.python.org/zh-cn/3/library/time.html#module-time) 模块的 `time.strftime(fmt, d.timetuple())`，但是并非所有对象都支持 `timetuple()` 方法。

对于 [`datetime.strptime()`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.datetime.strptime) 类方法，默认值为 `1900-01-01T00:00:00.000`: 任何未在格式字符串中指定的部分都将从默认值中提取。 [4](https://docs.python.org/zh-cn/3/library/datetime.html#id8)
...
对于 [`time`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.time) 对象，年、月、日的格式代码不应被使用，因为 [`time`](https://docs.python.org/zh-cn/3/library/time.html#module-time) 对象没有这些值。 如果它们被使用，则年份将被替换为 `1900`，而月和日将被替换为 `1`。

对于 [`date`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.date) 对象，时、分、秒和微秒的格式代码不应被使用，因为 [`date`](https://docs.python.org/zh-cn/3/library/datetime.html#datetime.date) 对象没有这些值。 如果它们被使用，则它们都将被替换为 `0`。

出于相同的原因，对于包含当前区域设置字符集所无法表示的 Unicode 码位的格式字符串的处理方式也取决于具体平台。 在某些平台上这样的码位会不加修改地原样输出，而在其他平台上 `strftime` 则可能引发 [`UnicodeError`](https://docs.python.org/zh-cn/3/library/exceptions.html#UnicodeError) 或只返回一个空字符串。